...
function matrixReolad(){
	var param = 'thAction=MATRIX_RELOAD&ObjectKey=<%=se.getRequest().getParameter("ObjectKey")%>&FatherKey=<%=se.getRequest().getParameter("FatherKey")%>';
	var url = parent.opener.location.href+'?'+ param;
...
function okButtonSmontaggio() {
	...
	if (magKit != "" ) {
		...
		runActionDirect('SMONT_RIGHE', 'action_submit', className, null, 'same', 'no');
	} else
		alert("Attenzione: 'Magazzino' è un campo obbligatorio!");
